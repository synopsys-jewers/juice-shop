...
module.exports = function searchProducts () {
  return ({ query }, res, next) => {
    ...
    models.sequelize.query('SELECT * FROM Products WHERE ((name LIKE :criteria OR description LIKE :criteriaWildcard) AND deletedAt IS NULL) ORDER BY name',
    {
      replacements: {
        criteria: criteria,
        criteriaWildcard: '%' + criteria + '%'
      }
    })
      .then(([products, query]) => {
